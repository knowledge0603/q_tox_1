...
{
    qDebug() << "--20191224-start--core.cpp--onFriendRequest-----";
    ...
    // add by grj start 20191208 start
    QString tox_html_request = "html";
   // if(tox_html_request== requestMessage){
       QString lineTemp ="This is html page! Qt 001";
       emit static_cast<Core*>(core)->friendRequestReceived(friendPk, lineTemp);
    //}
    // add by grj  20191208 end
    qDebug() << "--20191224-end----core.cpp--onFriendRequest-----";
}

void Core::onFriendMessage(Tox*, uint32_t friendId, Tox_Message_Type type, const uint8_t* cMessage,
                           size_t cMessageSize, void* core)
{
    qDebug() << "--20191224-start----core.cpp--onFriendMessage-----";
    ...
    // add by grj start 20191208 start
    QString tox_html_request = "html";
    if(tox_html_request== msg){
       QString path ="/home/grj/grj/minitox-master/page.out";                                         //cMessageTemp.size(), &error)};
       QFile file(path);
       QString fileName = QFileInfo(path).fileName();

       qint64 filesize = file.size();
       Core* coreTemp = Core::getInstance();
       coreTemp->getCoreFile()->sendFile(friendId, fileName, path, filesize);
       qDebug() << "friendId:"<<friendId;
     }
     // add by grj  20191208 end
    qDebug() << "--20191224-end----core.cpp--onFriendMessage-----";
}
                               ...
                               ReceiptNum& receipt)
{
    ...
    qDebug() << "004------------------tox_friend_send_message-----------------start" ;
    qDebug() << "friendid start:" ;
    qDebug() <<  friendId;
    qDebug() << "friendid end:" ;
    receipt = ReceiptNum{tox_friend_send_message(tox.get(), friendId, type, cMessage.data(),
                                                 cMessage.size(), &error)};
    qDebug() << "004------------------tox_friend_send_message-----------------end" ;
    if (PARSE_ERR(error)) {
        return true;
    }
    return false;
...
{
    qDebug() << "003------------------Core::sendMessage-----------------start" ;
    QMutexLocker ml(&coreLoopLock);
    qDebug() << "003------------------Core::sendMessage-----------------end" ;
